...
public final class PatternMapDrawable implements IDrawable
{
	...
	public PatternMapDrawable(IMap map, IPattern pattern, double size, double spacingScale, String fgColor, String bgColor)
	{
		_map = map;
		_pattern = pattern;
		_size = size;
		...
		if((fgColor != null) && (fgColor.length() > 0))
		{
			...
			{
				_fgColor = null;
				_fgColorIndex = SceneContext.parseColorIndex(fgColor.substring(1));
			}
			else
			{
		...
		if((bgColor != null) && (bgColor.length() > 0))
		{
			...
			{
				_bgColor = null;
				_bgColorIndex = SceneContext.parseColorIndex(bgColor.substring(1));
			}
			else
			{
