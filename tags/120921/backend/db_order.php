<?php
	...
	class OrderDB
	{
		...
		function loadBarcode($row)
		{
			...
			$result->dateUsed = strtotime($row['date_used']);
		...
		{
			$query = sprintf(
					"INSERT INTO barcodes(barcode, product_id, date_created, master, date_used) VALUES ('%s', %d, NOW(), '%s', '%s')",
					mysql_escape_string($barcode->barcode),
					$barcode->productId,
					mysql_escape_string($barcode->master),
					date("Y-m-d H:i:s", $barcode->dateUsed)
				);
		...
		{
			$query = sprintf(
					"UPDATE barcodes SET product_id=%d, master='%s', date_used='%s' WHERE barcode = '%s'",
					$barcode->productId,
					mysql_escape_string($barcode->master),
					date("Y-m-d H:i:s", $barcode->dateUsed),
					mysql_escape_string($barcode->barcode)
		...
		{			
			...
			{
				if($first) { $first = false; } else { $query = $query.", "; }
				$query = $query.sprintf("shape_id='%s'", mysql_escape_string($orderItem->color));
			}
		...
		function getOrderItemsByOrderId($orderId, $processingStageId = NULL, $withData = false)
		{
