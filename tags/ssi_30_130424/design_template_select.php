<?php
	...
	if(isset($_POST['next']))
	{
		...
		$_design_db->setDesignJSON($_design_id, $template->designJSON);
	...
	function writeTemplateImage($template, $selectedTemplateId)
	{
		...
		echo sprintf('<td class="%s" id="template_cell_%d" ondblclick="selectTemplate(%d,true)" onclick="selectTemplate(%d)" ><img id="template_image_%d" src="design_part/get_image.php?id=%s"/><br>%s</td>',
				$cssClass,
				$template->id,
				$template->id,
				$template->id,
				$template->id,
				ImageDB::TYPE_THUMBNAIL.'.'.$template->getPreviewImageId(),
	...
	function writeCategories()
	{
		...
		$list = $_design_db->getTemplateCategoryListForDesign($design);
		foreach($list as $item)
		{
