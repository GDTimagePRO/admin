...
<head>
	...
	<style>
		...
		.control_group_header, .control_group_header_selected 
		{
			...
			background: #3BAAE3 url(css/themes/<?php echo $_session->getActiveThemeName(); ?>/images/ui-bg_glass_50_3baae3_1x400.png) 50% 50% repeat-x;
			font-weight: bold;
			color: white;			
		...
		{
			background: #5d9bbb url(css/themes/<?php echo $_session->getActiveThemeName(); ?>/images/ui-bg_glass_50_3baae3_1x400_selected.png) 50% 50% repeat-x;
		}
...
<?php 
// 	if(!is_null($orderItemConfig->inkColor))
// 	{
// 		echo '_system.changeInkColour("'.$orderItemConfig->inkColor.'");';
// 	}			
?>
			...
			$("#nextButton").button().click( function() {
				...
				_system.invokeWhenReady(function() {					
					...
					//var trace = renderOffscreen(
					//		_outputImageWidth_trace, _outputImageHeight_trace, 
					//		_outputImageFrameWidth_trace, _outputImageFrameHeight_trace,
					//		true
					//	);

					var previewCanvas = renderOffscreen(
						_outputImageWidth_preview, _outputImageHeight_preview, 
						_outputImageWidth_preview, _outputImageHeight_preview 
					);

					//trace.processLog();
					//var scl = $.toJSON(trace.log);
					
					var sclUploaded = true;
					var imageUploaded = false;					
					
					//_system.setDesignSCL(scl, function(success, result) {
					//	if(!success || !result)
					//	{
					//		alert("Error uploading scl.");
					//	}
					//	sclUploaded = true;
					//	
					//	if(sclUploaded && imageUploaded) window.location = _nav_next;
					//});
