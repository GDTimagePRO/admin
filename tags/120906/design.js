...
function drawBorder(border,context){
	if(border.show){
		...
		//console.log("border.x: "+ border.x);
		//console.log("border.y:: "+border.y);
		//console.log("border.width: "+border.width);
		//console.log("border.height: "+border.height);
		_mapParams = { x1:border.x, y1:border.y, x2:parseInt(border.x)+parseInt(border.width), y2:parseInt(border.y)+parseInt(border.height), r:radius, s:0.8 };
...
function newTextLineServerCall(index){
	...
	xmlhttp.send();
	textLines[index].id = xmlhttp.responseText;
}
...
function newGraphicServerCall(index,image_id){
	...
	xmlhttp.open("GET","newgraphic.php?order_id="+$('#orderitem_id').html()+"&image_id="+image_id,false);
	xmlhttp.send();
	images[index].id = xmlhttp.responseText;
}
...
function newBorderServerCall(index,SelectedBorder){
	...
	xmlhttp.open("GET","newborder.php?order_id="+$('#orderitem_id').html()+"&x="+SelectedBorder.x+"&y="+SelectedBorder.y+"&width="+SelectedBorder.width+"&height="+SelectedBorder.height+"&type_id="+SelectedBorder.type_id+"&style_id="+SelectedBorder.style_id+"&sides="+SelectedBorder.sides+"&radius="+SelectedBorder.radius+"&line_width="+SelectedBorder.line_width,false);
	xmlhttp.send();
	borders[index].id = xmlhttp.responseText;
}
...
function newLineServerCall(index,SelectedLine){
	...
	xmlhttp.open("GET","newline.php?order_id="+$('#orderitem_id').html()+"&x="+SelectedLine.x+"&y="+SelectedLine.y+"&x2="+SelectedLine.x2+"&y2="+SelectedLine.y2+"&type_id="+SelectedLine.type_id+"&line_width="+SelectedLine.line_width,false);
	xmlhttp.send();
	lines[index].id = xmlhttp.responseText;
}

function selectTextLine(id)	{
	if(!textLines[id].lock){
		mySel = textLines[id];
	}
	invalidate(false);
}
...
function newTableServerCall(index,SelectedTable){
	...
	xmlhttp.open("GET","newtable.php?order_id="+$('#orderitem_id').html()+"&x="+SelectedTable.x+"&y="+SelectedTable.y+"&width="+SelectedTable.width+"&height="+SelectedTable.height+"&rows="+SelectedTable.rows+"&columns="+SelectedTable.columns+"&border="+SelectedTable.border,false);
	xmlhttp.send();
	tables[index].id = xmlhttp.responseText;
}
...
function initBorderPreview(){
	...
	image.src = "getproductimage.php?id="+$('#orderitem_id').html()+"&b="+new Date().getMilliseconds();
...
function deleteGraphic(index){
	...
	xmlhttp.open("GET","deletegraphic.php?id="+image_id,true);
	xmlhttp.send();	
...
function deleteBorder(index){
	...
	xmlhttp.open("GET","deleteborder.php?id="+borders[index].id,true);
	xmlhttp.send();	
...
function deleteLine(index){
	...
	xmlhttp.open("GET","deleteline.php?id="+lines[index].id,true);
	xmlhttp.send();	
...
function deleteTable(index){
	...
	xmlhttp.open("GET","deletetable.php?id="+tables[index].id,true);
	xmlhttp.send();	
...
function changeColorPreview(){
	var color = $('#color').val();
	var source = "getproductimage.php?id="+$('#orderitem_id').html().replace(/^\s+|\s+$/g, '')+"&color="+color;
	//console.log(source);
	$('#colourpreview').attr("src",source);
}
...
function uploadGraphicCheck(element){
	var fileElement = element;
	var file = element.files[0];
	//console.log('uploadGraphicCheck');
	if(file){
...
function uploadGraphicSubmit(){
	//console.log("upload graphic submit");
	//$('#uploadGraphicFile').bind('fileuploaddone',uploadGraphicComplete);
	//$('#uploadGraphicFile').bind('fileuploadprogress',uploadProgress);
	//$('#uploadGraphicFile').bind('fileuploadstart',function (e){ //console.log("Upload Started");});
	//$('#uploadGraphicFile').fileupload({url: 'uploadgraphic.php',type: 'POST', start: function (e){ //console.log("Upload Started");}})
  //var xhr;
		if (browserName!="Microsoft Internet Explorer")
		  {// code for IE7+, Firefox, Chrome, Opera, Safari
		  ...
		  {// code for IE6, IE5
		    ...
		    var eventHandler = function () {
		            ...
		            uploadGraphicComplete(null,null);
...
function uploadGraphicComplete(e,data){
	changeImageCategory(1);
	showTab('#selectgraphictab');
}
...
function saveDesign(){
	if(toSave){
		...
		var s = "{ \"orderitem_id\": "+$('#orderitem_id').html()+", \"color\":\""+textColor+"\", \"textlines\": [";
		for(var i=0;i<numLines;i++){
			if(textLines[i].show){
		...
		if(images.length>0&&images[images.length-1]!=undefined&&images[images.length-1].show){
			s+=images[images.length-1].toJson();
		}
		s+="], \"borders\": [";
		for(var i=0;i<borders.length-1;i++){
			if(borders[i].show){
				//console.log("Saving border: "+borders[i].type_id);
				s+=borders[i].toJson()+",";
			}
		}
		if(borders.length>=1&&borders[borders.length-1]!=undefined&&borders[borders.length-1].show){
			s+=borders[borders.length-1].toJson();
			//console.log("Saving border: "+borders[borders.length-1].type_id);
		}
		//s+="]}";
		s+="], \"lines\": [";
		...
		s+="], \"data\": \"";
		drawToGhost();
		s+=ghostcanvas.toDataURL();
		s+="\"}";
		/*xmlhttp.onreadystatechange=function()
	  {
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	    {
	    $('#error_checking').html(xmlhttp.responseText);
	    }
	  }*/
 ...
 *****************************************************************************************************************************************/
 function Border()
  ...
  function LineMap( destX1, destY1, destX2, destY2, srcX )
  {
    ...
    if(!displayed){
	    //console.log("-----------LineMap--------------");
	    //console.log("destX2: "+destX2);
	    //console.log("destX1: "+destX1);
	    //console.log("destY2: "+destY2);
	    //console.log("destY1: "+destY1);
    ...
    this.transform = function(x ,y)
    {
      x = x - srcX;         
      return [
        x * ca + y * sa + parseInt(destX1) ,  
        x * sa - y * ca + parseInt(destY1) ,
        angle,
      ];
    };
  }
  ...
  function roundedRectangleMap(x1, y1, x2, y2, radius, srcScale)
  {
    ...
    if(width > 0)
    {
      map.addMap(new LineMap(
        parseInt(x1) + parseInt(radius), y1,
        x2 - radius, y1, 
        map.range[1] ));
    }        
    ...
    if(height > 0)
    {
      ...
      map.addMap(new LineMap(
        x2, parseInt(y1) + parseInt(radius), 
        x2, y2 - radius, 
        map.range[1] ));
    }        
    ...
    if(width > 0)
    {
      map.addMap(new LineMap(
        x2 - radius, y2, 
        parseInt(x1) + parseInt(radius), y2,
        map.range[1] ));
    }        
    ...
    if(height > 0)
    {
      map.addMap(new LineMap(
        x1, y2 - radius, 
        x1, parseInt(y1) + parseInt(radius), 
        map.range[1] ));
    }        
    ...
    this.border = function(map, context, size, spacingScale)
    {
      ...
      //var height = _h;
          ...
          var segments = Math.floor((map.range[1] - map.range[0]) / spacing);
          spacing = (map.range[1] - map.range[0]) / segments;
          //console.log("------------PatternRope---------------");
          //console.log("map.range[0]: "+map.range[0] );
          //console.log("map.range[1]: "+map.range[1] );
 ...
 function PatternLines(lineSpecs)
  {
    ...
    this.border = function(map, context, size, spacingScale)
    {
      ...
      if(!displayed){
	      //console.log("---------PatternLines.border------");
	      //console.log("offsetX: "+offsetX);
	      //console.log("spacing: "+spacing);
	      //console.log("segments: "+segments);
	      //console.log("lineSpecs[0].distance: "+lineSpecs[0].distance);
	      //console.log("lineSpecs[0].size: "+lineSpecs[0].size);
	  }
      for(var iLineSpec=0; iLineSpec <lineSpecs.length; iLineSpec++)
      {
        var lineBottom = lineSpecs[iLineSpec].distance * scale;
