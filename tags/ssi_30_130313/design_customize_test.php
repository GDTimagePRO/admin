		...
		$(document).ready(function() {
			...
			$("#nextButton").button().click( function() {
				...
				_system.invokeWhenReady(function() {					
					...
					trace.processLog();
					$('#traceLog').val($.toJSON(trace.log));
					window.open(backBuffer.toDataURL(), "toDataURL() image", "width=600, height=200");
				});
			});			
		});
...
//-------------------------------------------------------------------------------------------------------------------------
		...
		function doStuff(X)
		{
			var sc = new ScriptContainer(_system.elements[0]);
			sc.addBinding("onTest");
			sc.addBinding("onFilter");

			sc.setSource( "function onTest(a,b) { alert(a + ' ' + b + ' ' + this.val()); }" );

			if(sc.onTest) sc.onTest("Hello", "World");
			if(sc.onFilter) alert(sc.onFilter("Hello"));
			
			sc.setSource( "function onFilter(x) { return x.toUpperCase(); }" );

			if(sc.onTest) sc.onTest("Hello", "World");
			if(sc.onFilter) alert(sc.onFilter("Hello"));
			
						
			//alert(sc.onFilter(["Hello"], "Default"));
			
			//var s = ScriptContainerAPI("#q");
			//alert(s.val('ABC').val());
		}
...
<body unselectable="on" class="unselectable">
	...
	<textarea id="traceLog" style="width:500px; height:100px"></textarea>
