<?php 
	...
	function writeBarcodeList()
	{
		...
		writeTableHeader('Product', 3, $orderBy);
		writeTableHeader('Template', 4, $orderBy);
		writeTableHeader('Category', 5, $orderBy);
		writeTableHeader('Date Created', 6, $orderBy);
		writeTableHeader('Date Used', 7, $orderBy);
		echo '<td></td>';
		echo '<td></td>';
		echo '</tr>';
		...
		while($row = mysql_fetch_assoc($result))
		{
			...
			echo '</td><td>';
			echo htmlspecialchars($row['product_id']).": ".htmlspecialchars($productCode);
			echo '</td><td>';
