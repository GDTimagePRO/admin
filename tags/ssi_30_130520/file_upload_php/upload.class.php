    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
            	...
            	$file->dbImageId = isset($_REQUEST['_image_id']) ? $_REQUEST['_image_id'] : -1;
            	$dbImageCategoryId = isset($_REQUEST['_category_id']) ? $_REQUEST['_category_id'] : 1;
            	            	
            	$imageMaybeTrueColor = imagecreatefromstring(file_get_contents($file_path));
            	...
            	$imageTrueColorData = ob_get_contents();
				ob_end_clean();            	 
				...
				if($_user_id != "")
				{
					if($file->dbImageId == -1)
					{
						$file->error = 'User images are not supported atm.';
// 						$file->dbImageId = $_image_db->createImage(
// 								$dbImageCategoryId,
// 								$_user_id,
// 								$file->name,
// 								$imageTrueColorData
...
// 						}		
					...
					{
						if(!$_image_db->setImageData(
								$file->dbImageId,
								file_get_contents($file_path)
							))
						{
							$file->error = 'db error : ' . mysql_error();
						}		
					}
