...
function UIPanel(propContainerId, listContainerId)
{
    ...
    var getControlHTML = function(group, index, tabIndex)
    {
    	var control = group.members[index];
    	var common = tabIndex ? "tabindex=\"" + tabIndex + "\"" : "" ;
    	
        switch(control.getType())
        {
        case UIControl.TYPE_TEXT:
        	return '<input class="control_input" id="' + control.getId() + '" type="text" ' + common + ' style="padding-left:2px">';
        case UIControl.TYPE_NUMBER: 
            return '<input class="control_input" id="' + control.getId() + '" type="text" ' + common + ' >';
            
        case UIControl.TYPE_LIST:
            return '<select class="control_input" id="' + control.getId() + '" ' + common + ' ></select>';
            
        case UIControl.TYPE_BUTTON:
            return '<button class="control_input" id="' + control.getId() + '" ' + common + ' ></button>';
        }
...
//    	}
		...
		for(var i in groups)
        {
    		...
    		if(group.params.element.displayOptions.visibility == 0)
    		{
	    		...
	    		case UIPanel.TEMPLATE_TEXT_ELLIPSE_1:
	    			...
	    			html += '<div class="control_text_container" style="margin-left:7px">' + getControlHTML(group, iMember, tabIndex++) + '</div>';
		    		html += '</div>';
    ...
    var selectGroup_selectTimeout = null;
    ...
    this.selectGroup = function(group)
    {
    	...
    	if(previousGroup)
    	{
	    	switch(previousGroup.template)
	    	{
	    		case UIPanel.TEMPLATE_TEXT_LINE_1:
	    		case UIPanel.TEMPLATE_TEXT_CIRCLE_1:
	    		case UIPanel.TEMPLATE_TEXT_ELLIPSE_1:
	    		{
	    			iMember = findGroupMember(previousGroup, "text");		
	    			var control = previousGroup.members[iMember];    			
	    			var id = '#' + control.getId();
	    			var select = $(id);
    				select.removeClass("control_group_input_selected");
	    		}
	    		break;
	    	}
    	}
    	...
    	if(group)
    	{
	    	switch(group.template)
	    	{
	    		...
	    		case UIPanel.TEMPLATE_TEXT_ELLIPSE_1:
	    		{
	    			...
	    			var id = '#' + control.getId();
	    			var select = $(id);
    				if(UIPanel.MOBILE_MODE) select.addClass("control_group_input_selected");
	    			clearTimeout(selectGroup_selectTimeout);
	    			selectGroup_selectTimeout = setTimeout(function() {select.focus();}, 1);	    				
	    		}
	    		break;
	    	}
    	}
