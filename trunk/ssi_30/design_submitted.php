<?php
	...
	$orderItemId = $_session->getActiveOrderItemId();
	if(empty($orderItemId)) $_system->loginRedirect();
	
	if(submitOrderItem($orderItemId))
	{
		$designImageId = ImageDB::TYPE_THUMBNAIL.'.'.OrderItem::previewImageId($orderItemId);
		clearSelectedOrderItem();
		$returnUrl = $_session->getReturnUrl();
		if($returnUrl != "")
		{
			Header("location: ".$returnUrl."?OrderId=".$designImageId);
			setcookie("redirect", $returnUrl."?OrderId=".$designImageId);
			exit;
		}
	}
	else
