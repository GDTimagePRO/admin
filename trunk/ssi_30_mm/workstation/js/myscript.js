...
function print(template, width, height) {
	...
	if(count>0){
		...
		if (typeof width != 'undefined' && typeof height != 'undefined' && !isNaN(width) && !isNaN(height)) {
			dataString = "s=0&template="+template+"&id="+total+"&user="+$('#userID').html()+"&width="+width+"&height="+height;
		} else {
			dataString = "s=0&template="+template+"&id="+total+"&user="+$('#userID').html();
		}
