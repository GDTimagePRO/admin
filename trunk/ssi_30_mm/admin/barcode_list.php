<?php 
	...
	function writeBarcodeList()
	{
		...
		$query = 
			"SELECT
				...
				b.master AS master,
				b.date_used AS date_used,
				b.config_json as config_json, 
		...
		switch(abs($orderBy))
		{
			default:
				$orderBy = 1;
			case 1:
				$query .= 'ORDER BY customer_id ' . $orderType . ', barcode ASC ';
				break;
				 
			case 2:
				$query .= 'ORDER BY customer_id ASC, barcode ' . $orderType;
				break; 
				
			case 3:
				$query .= 'ORDER BY customer_id, master '.$orderType.', date_created ASC, barcode ASC ';
				break;
					
			case 4:
				$query .= 'ORDER BY date_created '.$orderType.', customer_id, barcode ASC ';
				break;
			
			//case 5:
			//	$query .= 'ORDER BY customer_id, date_used '.$orderType.', barcode ASC ';
			//	break;
		...
		echo '<tr>';
		echo '<td>Index</td>';
		writeTableHeader('Customer', 1, $orderBy);
		writeTableHeader('Barcode', 2, $orderBy);
		writeTableHeader('Master', 3, $orderBy);
		writeTableHeader('Date Created', 4, $orderBy);
		//writeTableHeader('Date Used', 5, $orderBy);
		writeTableHeader('JSON', 6, $orderBy);
		echo '<td></td>';
		echo '<td></td>';
		echo '</tr>';
		...
		while($row = mysql_fetch_assoc($result))
		{
			...
			echo '</td><td>';
			echo "<a href='barcode_edit.php?barcode=".$row['barcode']."&customerId=" . $row['customer_id'] . "'>Edit</a>";
			echo '</td><td>';
			echo "<a href='barcode_list.php?delete_barcode=".$row['barcode']."&customerId=" . $row['customer_id'] . "' onclick='deletePrompt();'>Delete</a>";
			echo '</td><tr>';
		}
		echo '</table>';
	...
	{
		$barDelete = $_order_db->getBarcodeByBarcode($_GET['customerId'], $_GET['delete_barcode']);
		if(!is_null($barDelete))
		{
			$_order_db->deleteBarcode($_GET['customerId'], $barDelete->barcode);
		}
	}
