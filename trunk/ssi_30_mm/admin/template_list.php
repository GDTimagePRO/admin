<?php 
	...
	function writeTableHeader($text, $sortIndex, $selectedSortIndex)
	{
 	...
 	function writeTemplateList() 	
 	{
 		...
 		$query =	'SELECT dt.id, dt.name, dt.category_id, dtc.name as category_name, dt.product_type_id, ddt.description as product_type_name, c.description as customer_description '.
 					'FROM design_templates AS dt '.
 					'LEFT JOIN (design_template_categories as dtc, default_design_templates as ddt, customers as c) ON '.
 					'(dt.category_id = dtc.id) AND (dt.product_type_id = ddt.product_type_id) AND (dtc.customer_id = c.id)';
 		...
 		switch(abs($orderBy))
 		{
 			...
 			case 3:
 				$query .= 'ORDER BY c.id '.$orderType.', dtc.name '.$orderType.', dt.id ASC ';
 				break;
		...
		while($row = mysql_fetch_assoc($result))
 		{
			...
			if(!is_null($customer_description)) $category_name = $customer_description . ' -> ' . $category_name;  
			...
			echo '</td><td>';
			echo '<img style="margin:2px; border-width:1px;border-style:solid;" src="' .Settings::getImageUrl(ResourceManager::TYPE_THUMBNAIL.'.'.$preview_image_id). '">';		
			echo '</td><td>';
