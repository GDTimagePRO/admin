...
function UIPanel(propContainerId, listContainerId)
{
    ...
    var getControlHTML = function(group, index)
    {
        ...
        {
        case UIControl.TYPE_TEXT:
	        if (group.params.element.displayOptions.maxSize > 0) {
				return '<input class="control_input" id="' + control.getId() + '" type="text" style="padding-left:2px" maxlength="' + group.params.element.displayOptions.maxSize + '">';
			} else {
    	...
    	this.create = function()
    	{
    		...
    		else
    		{
				...
				html +=
					...
					'<div id="text_angle" style="width:57px;display:inline-block;vertical-align:top; padding-top:6px;">&nbsp;&nbsp;&nbsp;&nbsp;Rotate:</div>' +
		...
		for(var i in groups)
        {
    		...
    		if(group.params.element.displayOptions.visibility == 0)
    		{
	    		...
	    		case UIPanel.TEMPLATE_TEXT_ELLIPSE_1:
		    		...
		    		{
		    			addControl(group, findGroupMember(group, "text"));
		    		}
    		...
    		if(group.params.element.displayOptions.visibility == 0)
    		{
	        	...
	        	case UIPanel.TEMPLATE_IMAGE_1:
	    			...
	    			if(group.params.element.getEditAllowMove())
	        		{
		    			...
		    			html += '<br><div class="control_label" tooltip="angle">Rotate:</div>';
		    			html += '<div class="control_number_container" style="padding-left:8px;">' + getControlHTML(group, iMemberAngle) + '</div>';

		    			var iMemberSize = findGroupMember(group, "size");
		    			initTodo.push({g:group, i:iMemberSize});    			
		    			html += '<div class="control_label" tooltip="angle" style="padding-left:8px;">Size:</div>';
		    			html += '<div class="control_number_container" style="padding-left:8px;">' + getControlHTML(group, iMemberSize) + '</div><span style="padding-left:1px;">%</span>';

		    			var iMemberCenterX = findGroupMember(group, "centerX");
		    			initTodo.push({g:group, i:iMemberCenterX});    			
		    			//html += '<br><div class="control_label" tooltip="centerX" style="padding-left:8px;">Left-Right:</div>';
		    			html += '<br><div class="control_label" tooltip="centerX">Left-Right:</div>';
		    			html += '<div class="control_number_container" style="padding-left:8px;">' + getControlHTML(group, iMemberCenterX) + '</div>';

		    			var iMemberCenterY = findGroupMember(group, "centerY");
		    			initTodo.push({g:group, i:iMemberCenterY});    			
		    			html += '<div class="control_label" tooltip="centerY" style="padding-left:8px;">Up-Down:</div>';
		    			html += '<div class="control_number_container" style="padding-left:8px;">' + getControlHTML(group, iMemberCenterY) + '</div>';
	        		}
