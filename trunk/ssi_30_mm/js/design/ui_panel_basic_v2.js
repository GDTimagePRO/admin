...
function UIPanel(propContainerId, listContainerId)
{
	...
	function ScriptInterface(group, onAdd)
	{
	    this.addControl = function(name, type, params)
	    {
	    	group.addControl(name, type, params);
	    	onAdd(group, group.members.length-1);
	    };
	}
...
//    	}
    ...
    var ButtonBar = function(group)
    {
    	...
    	this.create = function()
    	{
    		...
    		return;//Added to hide extra options
    ...
    var createOptionalControls_AddCell = function(group, width)
    {
		var body = $("#" + group.updateHTML.bodyId);
		var tr = body.find("#uglyTextLineTableRow");		
		if(!width) width = '1px';
		var td = $('<td style="padding:0;width:' + width + ';"></td>');    				
		tr.append(td);
		return td;
    };
    ...
    {
    	var canvas = $('<canvas>');
    	var scene = new Scene(canvas[0], 300, 300);
    	...
    	this.redraw = function()
    	{
    		...
    		scene.offsetX = scene.width / 2.0;
    		scene.offsetY = scene.height / 2.0;
    		if(this.onUpdate ) this.onUpdate(scene); 
    		scene.redraw();  
    	};
    };
    
	var IMG_URL = "images/ui_panel_button/";

	var createOptionalControls_TextElement = function(group)
    {
    	//group.params.element.config.controls = ["fontBold","fontItalic", "color"];
    	
    	var controls = group.params.element.config.controls;    	
    	if(!controls) return null;
    	
    	var makeButton = function(control)
    	{
    		if((control == "fontBold") || (controls[control] == "fontBold"))
    		{
    			var button = $('<img style="width:25px;height:25px">');
				makeTooltip(button, "Toggle bold", elementTooltips);
    			var updateIcon = function()
    			{
    				if(group.params.element.getBold()) button.attr("src", IMG_URL + "boldtext_icon_true.png");    				
    				else button.attr("src", IMG_URL + "boldtext_icon_false.png");    				
    			};
    			...
    			button.click(function(){
    				group.params.element.setBold(!group.params.element.getBold());
					if(_system.scene) _system.scene.redraw();
    				updateIcon();
    			});
    			createOptionalControls_AddCell(group).append(button);
    		}
    		else if((control == "fontItalic") || (controls[control] == "fontItalic"))
    		{
    			var button = $('<img style="width:25px;height:25px">');
				makeTooltip(button, "Toggle italic", elementTooltips);
    			var updateIcon = function()
    			{
    				if(group.params.element.getItalic()) button.attr("src", IMG_URL + "italicizetext_icon_true.png");    				
    				else button.attr("src", IMG_URL + "italicizetext_icon_false.png");
    			};
    		...
    		else if((control == "color") || (controls[control] == "color"))
    		{
    			var button = $('<img id="' + group.updateHTML.bodyId + '_fontColor" style="width:37px;height:25px">');
				makeTooltip(button, "Change text color", elementTooltips);
				button.attr("src", IMG_URL + "color_drop_down.png");    				
				makeTooltip(button, "Change text color", elementTooltips);
    			createOptionalControls_AddCell(group).append(button);
    			
				if (_colors != null) {
					button.simpleColorPicker({
						onChangeColor: function(color)
						{
							group.params.element.setTextColor(color);
							if(_system.scene) _system.scene.redraw();
						},
						colors: _colors
					});
    ...
    var createOptionalControls_BorderElement = function(group, preview, drawable)
    {
    	//group.params.element.config.controls = ["color", "borderStyle"];
    	...
    	var makeButton = function(control)
    	{
    		if((control == "color") || (controls[control] == "color"))
    		{
    			var button = $('<img id="' + group.updateHTML.bodyId + '_fontColor" style="width:37px;height:25px">');
				button.attr("src", IMG_URL + "color_drop_down.png");    				
				makeTooltip(button, "Change border color", elementTooltips);
				createOptionalControls_AddCell(group).append(button);
				...
				if (_colors != null) {
					button.simpleColorPicker({
						onChangeColor: function(color)
						{
							group.params.element.setFGColor(color);
							preview.redraw();
							if(_system.scene) _system.scene.redraw();
						},
						colors: _colors
    		...
    		else if((control == "borderStyle") || (controls[control] == "borderStyle"))
    		{
    			var images = [];
    			var borders = [];
    			for(var i=0; i<BorderElement.BORDERS.length; i++)
    			{
    				if(i==1) continue;
    				borders.push(BorderElement.BORDERS[i]);
    				images.push(IMG_URL + "pattern_" + i + ".png");
    			}
    			
    			
    			var button = $('<img id="' + group.updateHTML.bodyId + '_fontColor" style="width:25px;height:25px">');
				button.attr("src", IMG_URL + "color_icon.png");
				makeTooltip(button, "Change border style", elementTooltips);
				
    			createOptionalControls_AddCell(group).append(button);
    			
    			var buttonArrow = $('<img style="width:13px;height:25px">');
    			buttonArrow.attr("src", IMG_URL + "drop_down_icon.png");    				
    			createOptionalControls_AddCell(group).append(buttonArrow);

    			var updateIcon = function()
    			{
    				var borderId = group.params.element.getBorder().id;
    				var src = images[0];
    				for(var i=0; i<borders.length; i++)
    				{
    					if(borderId == borders[i].id)
    					{
    						src = images[i];
    						break;
    					}
    			...
    			button.simpleImagePicker({
    				images: images,
    				onChangeImage: function(src)
    				{
        				for(var i=0; i<images.length; i++)
        				{
        					if(images[i] == src)        					
        					{
        						group.params.element.setBorder(borders[i]);
        						preview.redraw();
        						updateIcon();
        						if(_system.scene) _system.scene.redraw();
        					}
		...
		//drawable.pattern = BorderElement.BORDERS[2].make();
		//preview.getScene().getLayer(Scene.LAYER_FOREGROUND).add(drawable);
    };
    ...
    var createOptionalControls_ImageElement = function(group)
    {
    	//group.params.element.config.controls = ["filterSepia","flipHorizontal","flipVertical"];
    	
    	var setImageParam = function(paramIndex, value)
    	{
    		var image = group.params.element;
    		...
    		{
    			newParams.push(params.indexOf(_allParams[i]) >= 0 ? _allParams[i] : null);
    		}
    	...
    	var getImageParam = function(paramIndex)
    	{
    		var image = group.params.element;
    		var path = image.getImageId();
    		var params = ResourceId.getParams(path);
    		if(params == null) return false;
    		return params.indexOf(_allParams[paramIndex]) >= 0;
    	};

    	
    	
    	//group.params.element.config.controls = ["rotate"];
		var createFilterButton = function(iconName, tooltip, width, paramIndex) {
			var button = $('<img style="width:' + width + 'px;height:25px">');
			makeTooltip(button, tooltip, elementTooltips);
			var updateButtonImage = function()
			{
				if(getImageParam(paramIndex)) button.attr("src", IMG_URL + iconName + "_selected.png");
				else button.attr("src", IMG_URL + iconName + ".png");				
			};
			
			updateButtonImage();
			button.click(function()
			{
				setImageParam(paramIndex, !getImageParam(paramIndex));
				updateButtonImage();
			});
    	...
    	for(var control in controls)
    	{
    		if((control == "filterSepia") || (controls[control] == "filterSepia"))
    		{
    	    	createFilterButton("sepia_icon", "Sepia", 41, 0);
    		}
    		else if((control == "flipHorizontal") || (controls[control] == "flipHorizontal"))
    		{
    	    	createFilterButton("flip_horizontal_icon", "Horizontal flip", 25, 1);
    		}
    		else if((control == "flipVertical") || (controls[control] == "flipVertical"))
    		{
    	    	createFilterButton("flip_vertical_icon", "Vertical flip", 25, 2);
    		}
    	}
    	//var drawable = new PatternMapDrawable(null, null, 20, 1.0, null);
		//drawable.pattern = BorderElement.BORDERS[2].make();
    ...
    var updateHTML = this.updateHTML = function()
    {
		...
		var addControl = function(group, iMember)
		{
			initTodo.push({g:group, i:iMember});			
			html += '<div style="margin-left:7px">' + getControlHTML(group, iMember) + '</div>';
		};
		...
		for(var i in groups)
        {
    		...
    		if(group.params.element.displayOptions.visibility == 0)
    		{
	    		...
	    		case UIPanel.TEMPLATE_TEXT_ELLIPSE_1:
	    			...
	    			var groupId = id + '_body';
	    			html += '<div id="' + groupId + '" style="padding-top:8px">';
	    			...
	    			group.updateHTML = {bodyId:groupId};
		    		initTodoGroups.push(group);
	    			
		    		if(group.params.element.scriptContainer.onCustomUI)
		    		{
		    			group.params.element.scriptContainer.onCustomUI(
		    				new ScriptInterface(group, addControl)
		    			);
		    		}
		    		else
		    		{
		    			var iMember = findGroupMember(group, "text");
		    			initTodo.push({g:group, i:iMember});
		    	    	var control = group.members[iMember];
		    	    	var controlHTML = '<div style="display:block;margin-right:13px;"><input class="control_input" id="' + control.getId() + '" type="text" style="width:100%"></div>';
		    			html += '<table style="margin-left:9px;border-collapse: collapse;width:95%"><tr id="uglyTextLineTableRow"><td style="padding:0;width:auto">' + controlHTML + '</td></table>';
		    		}
	        		html += '</div>';					    		
	    			break;
	    			
	    		case UIPanel.TEMPLATE_BORDER_CIRCLE_1:
	    		case UIPanel.TEMPLATE_BORDER_ELLIPSE_1:
	    		case UIPanel.TEMPLATE_BORDER_RECTANGLE_1:
	    			...
	    			html += '<table style="margin-left:9px;border-collapse: collapse;width:95%"><tr id="uglyTextLineTableRow"></tr></table>';
	    			html += '</div>';					    		
	    			break;
	    		}
    		}
		...
		for(var i in groups)
        {
    		...
    		if(group.params.element.displayOptions.visibility == 0)
    		{
	        	...
	        	case UIPanel.TEMPLATE_IMAGE_1:
	    			...
	    			if(group.title) html += '<div class="control_label">' + htmlEncode(group.title) + '</div>';
	    			
	    			group.updateHTML = {bodyId:groupId};
		    		initTodoGroups.push(group);

		    		html += '<table style="border-collapse:collapse;"><tr><td>';
		    		html += '<table style="margin-left:9px;border-collapse: collapse;margin-top: 8px;"><tr id="uglyTextLineTableRow"></tr></table>';
		    		html += '</td><td>';

	    			if(group.params.element.getEditAllowMove())
	        		{
			    		var iMemberAngle = findGroupMember(group, "size");
		    			initTodo.push({g:group, i:iMemberAngle});    			
		    			html += '<div class="control_label resize" tooltip="centerX">';
		    			html += '<img style="position: relative;top: 8px; left: 6px;" src="' + IMG_URL + 'resize_icon.png">';
		    			html += '</div>';
		    			html += '<div class="control_number_container" style="padding-left:4px;">' + getControlHTML(group, iMemberAngle) + '</div><span style="padding-left:1px;">%</span>';
	        		}
	    			...
	    			if(group.params.element.getEditAllowMove())
	        		{
		    			...
		    			var iMemberCenterX = findGroupMember(group, "centerX");
		    			initTodo.push({g:group, i:iMemberCenterX});    			
		    			html += '<div class="control_label horizontal" tooltip="centerX">';
		    			html += '<img style="position: relative;top: 9px; left: 6px;" src="' + IMG_URL + 'horizontal_arrows.png">';
		    			html += '</div>';
		    			html += '<div class="control_number_container" style="padding-left:4px;">' + getControlHTML(group, iMemberCenterX) + '</div>';

		    			var iMemberCenterY = findGroupMember(group, "centerY");
		    			initTodo.push({g:group, i:iMemberCenterY});    			
		    			html += '<div class="control_label" tooltip="centerY" style="padding-left:12px;">';
		    			html += '<img style="position: relative;top: 9px; left: 6px;" src="' + IMG_URL + 'vertical_arrows.png">';
		    			html += '</div>';
		    			html += '<div class="control_number_container" style="padding-left:4px;">' + getControlHTML(group, iMemberCenterY) + '</div>';

		    			var iMemberAngle = findGroupMember(group, "angle");
		    			initTodo.push({g:group, i:iMemberAngle});    			
		    			html += '<div class="control_label" tooltip="angle" style="padding-left:13px;">';
		    			html += '<img style="position: relative;top: 9px; left: 4px;" src="' + IMG_URL + 'rotate_icon.png">';
		    			html += '</div>';
		    			html += '<div class="control_number_container" style="padding-left:4px;">' + getControlHTML(group, iMemberAngle) + '</div>';
        ...
        for(var iGroup in initTodoGroups)
        {
    		var group = initTodoGroups[iGroup];
        	var elementClass = group.params.element.className;
    		if(elementClass == "TextElement")
    		{
    			createOptionalControls_TextElement(group);
    		}
    		if(elementClass == "ImageElement")
    		{
    		
    			//var canvasWrapper = $('<div style="display:block;margin-right:7px;"></div>');
        		//canvasWrapper.append(preview.getCanvas());
        		
    			(function() {
        			iMember = findGroupMember(group, "change image");
        	    	var control = group.members[iMember];
        	    	
        			var button = $('<img style="width:79px;height:25px">');
    				button.attr("src", IMG_URL + "choosefile_icon.png");
    				button.click(function() {
    					if(control.onSet) control.onSet(null);
    				} );
    				createOptionalControls_AddCell(group).append(button);
    			})();
    			createOptionalControls_ImageElement(group);
        		createOptionalControls_AddCell(group);
    		}
    		else if(elementClass == "BorderElement")
    		{
    			(function(group)
    			{
        			var preview = new PreviewScene();
        			preview.getCanvas().css("width","100%");
        			preview.getCanvas().css("height","25px");
        			preview.getCanvas().addClass("shadow_box");

            		var drawable = new PatternMapDrawable(null, null, 20, 1.0, null);
            		drawable.pattern = BorderElement.BORDERS[2].make();
            		preview.getScene().getLayer(Scene.LAYER_FOREGROUND).add(drawable);
            		
            		preview.onUpdate = function(scene)
            		{
            			var ox = scene.width / 2.0 + 80; 
            			var oy = drawable.size / 2; 
            			drawable.map = new LineMap(-ox, oy, ox, oy, 0);
            			drawable.setFGColor(group.params.element.getFGColor());
    					var border = group.params.element.getBorder();
    					if(border.id == "fill")
    					{
                			drawable.pattern = BorderElement.getBorderFromDescriptor({ id: "solid" }).make();
    					}
    					else
    					{
            		...
            		createOptionalControls_AddCell(group, "auto").append(canvasWrapper);
        			createOptionalControls_BorderElement(group, preview, drawable);
        			preview.redraw();
    			})(group);
    		}
        }
