...
function PatternRibbon()
{
    ...
    this.border = function(map, context, size, spacingScale, fgColor, bgColor)
    {
        ...
        var spacingRibbon = 5;
        var rs = map.range[0];
        var segmentsRibbon = Math.floor((map.range[1] - rs) / spacingRibbon);
        var spacingRibbon = (map.range[1] - rs) / segmentsRibbon;
        
        context.fillStyle = fgColor;
        
        context.beginPath();
        var v_first = map.transform(0, ribbonHeight);
        ...
        {
            v = map.transform(i * spacingRibbon + rs, ribbonHeight);
            context.lineTo(v[0], v[1]);
        }

        v = map.transform(map.range[1], ribbonHeight);
        context.lineTo(v[0], v[1]);

        v = map.transform(map.range[1], 0);
        context.lineTo(v[0], v[1]);
        
        for(var i=segmentsRibbon-1; i>=0; i--)
        {
            v = map.transform(i * spacingRibbon + rs, 0);
            context.lineTo(v[0], v[1]);
        }
...
{
	this.state = ["HL", paddingTop, paddingBottom];
...
function PatternStars()
{
    ...
    function makeStar(scale)
    {
        return [
            ...
            { x :  -3.5 * scale, y :  -2.5 * scale },
            { x :   0.0 * scale, y : -12.0 * scale },
            { x :   3.5 * scale, y :  -2.5 * scale },
            { x :  12.5 * scale, y :  -2.5 * scale },
            { x :   4.5 * scale, y :   3.5 * scale },
            { x :   8.5 * scale, y :  12.5 * scale },
            { x :   0.0 * scale, y :   6.5 * scale },
            { x :  -8.5 * scale, y :  12.5 * scale },
            { x :  -4.5 * scale, y :   3.5 * scale }
        ];
    }
...
function PatternFill()
{
	...
	this.begin = function(map, context, size, spacingScale, fgColor, bgColor)
	{
        context.save();
        context.fillStyle = fgColor;
        context.beginPath();
		var v = map.transform(0, 0);        
		context.moveTo(v[0],v[1]);        
	};
	
	this.end = function(map, context, size, spacingScale, fgColor, bgColor)
	{
        context.closePath();
        context.fill();
        context.restore();        
    ...
    this.border = function(map, context, size, spacingScale, fgColor, bgColor)
    {
        var stride = size / 50.0;
        for(var i=1; i<=50; i++)
        {
    		var v = map.transform(i * stride, 0);        	
            context.lineTo(v[0], v[1]);
        }
    };
}
...
function TextPattern(text, font, scaleToFit, invert, bold, italic, minSize, alignment, verticalAlignment)
{
    ...
    if(!minSize) minSize = 8;

    this.state = ["Txt", text, font, scaleToFit, invert, bold, italic, minSize, alignment, verticalAlignment];
	
    
    if(typeof alignment === "undefined") alignment = 0; 
    if(typeof vertical_alignment === "undefined") vertical_alignment = 1; 
    ...
    {       
        ...
        var maxWidth = map.range[1] - map.range[0];
        
        //TODO: make this not suck!
        if(scaleToFit)
        {
        	var newMeasure =  context.measureText(text).width; //In case the font finished loading
	        if ( (oldWidth != maxWidth) || (oldSize != size) || (oldMeasure != newMeasure) ) 
	        {
	        	oldWidth = maxWidth;
	        	oldSize = size;
	        	oldMeasure = newMeasure; 
	        	
		        for (actSize = size; actSize > minSize; actSize--)
		    	{
		    		context.font = fontPre + Math.floor(actSize) + fontPost;
		    		var measured = context.measureText(text).width;
...
function PatternLines(lineSpecs)
{
	...
	for(var i=0; i<lineSpecs.length; i++)
	{
		var ls = lineSpecs[i];
		lineSpecStates.push([
		                     ls.radius ? ls.radius : 0, 
		                     ls.size ? ls.size : 0, 
		                     ls.distance ? ls.distance : 0, 
		                     ls.corner
		                     ]);
	}

	this.state = ["Lines", lineSpecStates];
