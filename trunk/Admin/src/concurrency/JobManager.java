...
{
	public static String INSTANCE_NAME = "Admin_JobManager"; 
	...
	public static class FinishedEvent {
		...
		public FinishedEvent(boolean error, String errorMessage) {
			this.error = error;
	...
	public interface IOnFinishedListener {
		public void finished(FinishedEvent e);
	}
	
	public interface IObserverListener
	{
		void onChange(Observer observer);
		void addOnFinishedListener(IOnFinishedListener listener);
		void removeOnFinishedListener(IOnFinishedListener listener);
	}
	...
	private final class Job extends Thread
	{
		...
		public Job(int id, UI ui, IObserverListener listener, IJobRunnable runnable)
		{
			_id = id;
			...
			while(true)
			{
				...
				try
				{
					...
					while(itr.hasNext())
					{
						...
						{
							job._listener.onChange(job._observer);
							System.out.println("Run without thread access");
						}
					}
				}
	...
	public synchronized int startJob(IJobRunnable runnable, IObserverListener listener, UI ui)
	{
		Job job = new Job( _idCount++, ui, listener, runnable );
		_jobs.add(job);
