...
{	
	...
	public static boolean removeImageFromCache(String id)
	{
		synchronized (_imageCache)
		{
			BufferedImage image = _imageCache.remove(id);
			if(image == null) return false;
			_cacheFIFO.remove(image);
			return true;
		}
	}
	...
	private final String _forceImageType;
	...
	private Font getFont(String name)
	{
		try
		{
			...
			if(result == null)
			{
					...
					if(name.indexOf('/') > -1)
					{
						path = ResourceManager.parseId(name).getPath();
					}
					else						
					{
						path = ResourceManager.getPath(ResourceManager.GROUP_LEGACY_FONTS, name +".ttf");
					}
					result = Font.createFont(Font.TRUETYPE_FONT, new File(path));
...
//			);d
	...
	public static Color parseColor(String value)
	{
		...
		else if ((value.length() == 8) && (value.charAt(0) != '#')) {
			return new Color(Integer.parseInt(value.substring(2, 4), 16), Integer.parseInt(value.substring(4, 6), 16), Integer.parseInt(value.substring(6, 8), 16), Integer.parseInt(value.substring(0, 2), 16));
		}
		else
		{
...
//		_g2d.setColor(Color.black);
	...
	public void drawImage(String descriptor, double width, double height, int type)
	{
		...
		try
		{			
			ResourceId rid = ResourceId.fromId(dp[1]);
			if((_forceImageType != null) && (_forceImageType.length() > 0)) rid.type = _forceImageType; 			
			String imageId = rid.getId();
			...
			if(image == null)
			{
				image = ImageIO.read(new File(rid.getPath()));
				addImageToCache(imageId, image);
			}
			
			AffineTransform t = new AffineTransform();
	        
			if(type == ImageDrawable.TYPE_CENTER)
			{
				...
				{
					//the frame is taller
					sizeMul = width / imgWidth; 	
				}
				else
				{
					//the frame is wider or equal
					sizeMul = height / imgHeight; 	
				}
				imgWidth *= sizeMul;
				imgHeight *= sizeMul;

				t.translate(-imgWidth / 2.0, -imgHeight / 2.0);	        
		        t.scale(sizeMul, sizeMul); 				
			}
			else
			{
	...
	public JavaCanvas(int width, int height, String forceImageType, boolean alphaChannel)
	{
		init(width, height, alphaChannel);
		_forceImageType = forceImageType;
	}
