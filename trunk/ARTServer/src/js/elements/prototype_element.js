var _prototypeElement = new function()
{
	...
	{
		var position = this.getPosition();
		var height = (position.y2 - position.y1) / 2.0;
		var width = (position.x2 - position.x1) / 2.0;
		return  Math.sqrt(height * height + width * width) / 2.0;		
	};
	...
	this.setSize = function(value)
	{
		value *= 2.0;
		var position = this.getPosition();		
		var centerX = (position.x1 + position.x2) / 2.0;
		var centerY = (position.y1 + position.y2) / 2.0;
		
		var adj = centerX - position.x1;
		var opp = centerY - position.y1;
		var hip = Math.sqrt(opp * opp + adj * adj);

		var offsetX = (adj / hip) * value;   
		var offsetY = (opp / hip) * value;   
    ...
    this.common_setState = function(state)
    {
		...
		this.config = state.config ? state.config : {};
