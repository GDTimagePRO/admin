...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BaseNopController
    {
        ...
        protected void PrepareCustomerInfoModel(CustomerInfoModel model, Customer customer, bool excludeProperties)
        {
            ...
            {
                Country countryHolder = _countryService.GetAllCountries().Where(c => c.DisplayOrder == 0).FirstOrDefault();
                if (countryHolder != null)
                {
                    model.CountryId = countryHolder.Id;
                }
                else
                {
        ...
        public ActionResult ReturnRequests()
        {
            ...
            var returnRequests = _orderService.SearchReturnRequests(customer.Id, 0, null, 0, int.MaxValue);
