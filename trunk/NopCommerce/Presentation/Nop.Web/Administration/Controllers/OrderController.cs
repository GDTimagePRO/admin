...
namespace Nop.Admin.Controllers
{
	...
	public partial class OrderController : BaseNopController
    {
        ...
        public OrderController(IOrderService orderService, 
            ...
            MeasureSettings measureSettings, PdfSettings pdfSettings, AddressSettings addressSettings,IImportManager importManager)
		{
        ...
        public ActionResult ExportCSVAll()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            try
            {
                ...
                var csv =  _exportManager.ExportOrdersToCSV(orders);
                byte[] bytes = new byte[csv.Length * sizeof(char)];
                System.Buffer.BlockCopy(csv.ToCharArray(), 0, bytes, 0, bytes.Length);
        ...
        public ActionResult ExportUPSXmlAll()
        {
            ...
            try
            {
                ...
                var xml = _exportManager.ExportOrdersToUPSXML(orders);
                return new XmlDownloadResult(xml, "orders.xml");
            }
            catch (Exception exc)
            {
        ...
        public ActionResult ImportShippingInfo(FormCollection form)
        {
            ...
            try
            {
                ...
                {
                    _importManager.ImportShippingInfoFromCSV(file.InputStream);
                }
                else
                {
                    ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
                    return RedirectToAction("List");
                    //return new XmlDownloadResult("Totally Failed, yo", "orders.xml");
                }
                SuccessNotification("Shipping Information Imported Successfully");//_localizationService.GetResource("Admin.Catalog.Products.Imported"));
                return RedirectToAction("List");
                //return new XmlDownloadResult("It worked!!", "orders.xml");
            }
            catch (Exception exc)
            {
                ErrorNotification(exc);
        ...
        public ActionResult PdfPickingSlipAll()
        {
            ...
            var orders = _orderService.SearchOrders(null, null, OrderStatus.Processing,
                      PaymentStatus.Paid, ShippingStatus.NotYetShipped, null, null, 0, int.MaxValue);

            byte[] bytes = null;
            using (var stream = new MemoryStream())
            {
                _pdfService.PrintPickingSlipsToPdf(stream, orders, _workContext.WorkingLanguage);
                bytes = stream.ToArray();
            }

            foreach (var order in orders)
            {
                _orderProcessingService.MarkOrderAsPending(order, false);
            }
