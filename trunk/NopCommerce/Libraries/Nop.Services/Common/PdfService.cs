...
namespace Nop.Services.Common
{
    ...
    public partial class PdfService : IPdfService
    {
        ...
        /// <param name="orders">Orders</param>
        /// <param name="lang">Language</param>
        public virtual void PrintPickingSlipsToPdf(Stream stream, IList<Order> orders, Language lang)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            ...
            int orderNum = 0;
            iTextSharp.text.Image image = iTextSharp.text.Image.GetInstance(new Uri(_localizationService.GetResource("PDFPickingSlip.ImageURI", lang.Id)));
            ...
            foreach (var order in orders)
            {
                ...
                titleParagraph.Add(new Phrase(_localizationService.GetResource("PDFPickingSlip.Title", lang.Id), titleFont));
                ...
                infoTable.AddCell(new Phrase(_localizationService.GetResource("PDFPickingSlip.BillingTitle", lang.Id), subtitleFont));
                infoTable.AddCell(new Phrase(_localizationService.GetResource("PDFPickingSlip.ShippingTitle", lang.Id), subtitleFont));
                if (_addressSettings.CompanyEnabled && !String.IsNullOrEmpty(order.BillingAddress.Company))
                ...
                {
                    if (item.AttributeDescription.Contains(":"))
                    {
                        PdfPCell nameParagraph = new PdfPCell();
                        nameParagraph.AddElement(new Phrase(item.ProductVariant.Product.Name));
                        char[] delimiterChars = { ':' };
                        String[] pieces = item.AttributeDescription.Split(delimiterChars);
                        String designID = pieces[1].Trim();
                        iTextSharp.text.Image productImage = null;
                        try
                        {
                            productImage = iTextSharp.text.Image.GetInstance(new Uri("http://in-stamp.com.loucks51.arvixevps.com/masonrow/design_part/get_image.php?thumbnail=true&id=" + designID));
                    ...
                    {
                        itemTable.AddCell(new Phrase(item.ProductVariant.Product.Name));
                    }
                    if (item.ProductVariant.Product.ProductCategories.Count > 0)
                    {
                        itemTable.AddCell(new Phrase(item.ProductVariant.Product.ProductCategories.First().Category.Name));
                    }
                    else
                    {
                        itemTable.AddCell(new Phrase(""));
                    }
                    itemTable.AddCell(new Phrase(item.ProductVariant.ManufacturerPartNumber));
