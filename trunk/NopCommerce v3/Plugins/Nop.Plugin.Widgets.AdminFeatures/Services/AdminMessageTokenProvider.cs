...
namespace Nop.Plugin.Widgets.AdminFeatures.Services
{
    class AdminMessageTokenProvider : MessageTokenProvider
    {
        ...
        public AdminMessageTokenProvider(ILanguageService languageService,
            ...
            TaxSettings taxSettings, IEventPublisher eventPublisher, GeneSysWidgetSettings genesysSettings)
            : base(languageService,
             localizationService,  dateTimeHelper,
             emailAccountService,
             priceFormatter,  currencyService, webHelper,
             workContext,  downloadService,
             orderService,  paymentService,
             productAttributeParser,
             storeSettings,  templatesSettings,
             emailAccountSettings,  catalogSettings,
             taxSettings,  eventPublisher)
        {
        ...
        protected override string ProductListToHtmlTable(Order order, int languageId)
        {
            var result = "";
            ...
            for (int i = 0; i <= table.Count - 1; i++)
            {
                ...
                {
                    sb.AppendLine("<br />");
                    if (opv.AttributeDescription.Contains("Order Id:")) 
                    {
                        char[] delimiterChars = { ':' };
                        String[] pieces = opv.AttributeDescription.Split(delimiterChars);
                        String designID = pieces[1].Trim();
                        sb.AppendFormat("<img src=\"http://{0}/design_part/get_image.php?thumbnail=true&id={1}\" />", _genesysSettings.GeneSysUrl, designID);
                    }
                    else
                    {
